...
/*
Classe leitora de arquivos .obj. Onde o arquivo contém os vários pontos, normais
e faces do objeto. No projeto trabalhamos com faces triangulares, ou seja, uma
face consiste em 3 pontos.
...
Obs: -  Para fins de abstração, as normais de cada ponto são ignoradas e
assumimos apenas uma normal para cada face.
...
class ObjReader {
   ...
   public:
            ...
            if (prefix == "mtllib") {
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    face.verticeIndice[i] =
                        std::stoi(token.substr(0, first)) - 1;
                    face.normalIndice[i] =
                        std::stoi(token.substr(second + 1)) - 1;
                }
                faces.push_back(face);
            }
        }
        for (const auto& face : faces) {
            std::vector<Ponto> points = {vertices[face.verticeIndice[0]],
                                         vertices[face.verticeIndice[1]],
                                         vertices[face.verticeIndice[2]]};
            facePoints.push_back(points);

            // faceNormals.push_back((normals[face.normalIndice[0]] +
            // normals[face.normalIndice[1]] + normals[face.normalIndice[2]])
            // / 3.0);
            faceNormals.push_back((points[1] - points[0])
    ...
    std::vector<std::vector<Ponto>> getFacePoints() { return facePoints; }

    std::vector<Vetor> getFaceNormals() { return faceNormals; }
    ...
    */
    ...
    Vetor getKd() { return curMaterial.kd; }

    // Método para retornar a cor do ambiente
    Vetor getKa() { return curMaterial.ka; }

    // Método para retornar o coeficiente especular (Refletência)
    Vetor getKe() { return curMaterial.ke; }
    ...
    Vetor getKs() { return curMaterial.ks; }

    // Método para retornar o indice de opacidade
    double getD() { return curMaterial.d; }

    // Método para retornar as coordenadas dos pontos
    std::vector<Ponto> getVertices() { return vertices; }

    std::vector<Vetor> getNormals() { return normals; }

    // Emite um output no terminal para cada face, com seus respectivos pontos
    // (x, y, z)
    void print_faces() {
        int i = 0;
