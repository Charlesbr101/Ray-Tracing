...
struct ColorData {
    ...
    bool positive() { return r > 0 || g > 0 || b > 0; }
};
...
struct ObjectData {
    ...
    vector<vector<Ponto>> facePoints;  // Only for meshes: list of faces, each
                                       // face is a list of vertices (Pontos)
    ...
    Ponto getPonto(string key) {
        auto p = vetorPointData[key];
...
struct SceneData {
    ...
    optional<CameraData> customBSPCamera;
};
...
static void indent(ostream& os) {
    for (int i = 0; i < IdentSpacing; i++) os << "    ";
}
static void printAsArray(ostream& os, Vetor v) {
    os << "[" << v.getX() << ", " << v.getY() << ", " << v.getZ() << "]";
}
static void printAsArray(ostream& os, Ponto v) {
    os << "[" << v.getX() << ", " << v.getY() << ", " << v.getZ() << "]";
}

ostream& operator<<(ostream& os, const ColorData& c) {
...
ostream& operator<<(ostream& os, const LightData& l) {
    ...
    os << "\"color\": " << l.color << ",\n";
    indent(os);
    os << "\"name\": \"" << extraData["name"] << "\"";

    for (auto& [k, v] : extraData)
        if (k != "name") {
            os << ",\n";
            indent(os);
            os << "\"" << k << "\": \"" << v << "\"";
...
ostream& operator<<(ostream& os, const SceneData& s) {
    ...
    materials.erase(unique(materials.begin(), materials.end(),
                           [](const MaterialData& a, const MaterialData& b) {
                               return a.name == b.name;
                           }),
                    materials.end());
